...
# Step 2: Create client with POLY_GNOSIS_SAFE signature type and funder=safe
client = ClobClient(
    host=config.CLOB_HOST,
    chain_id=config.CHAIN_ID,
    key=config.POLYGON_PRIVATE_KEY,
...
print(f"\nClient configured with signature_type=2 (POLY_GNOSIS_SAFE)")
...
# Step 4: Check balance/allowance for the Safe
bal = client.get_balance_allowance(BalanceAllowanceParams(asset_type=AssetType.COLLATERAL))
print(f"\nBalance/Allowance (POLY_GNOSIS_SAFE): {bal}")
